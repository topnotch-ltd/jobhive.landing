...
export interface PricingType extends LinkProps {
  title: string;
  icon: ReactNode;
...
interface PricingGridProps {
  features: PricingType[];
  classNames?: SlotsToClasses<FeaturesGridSlots>;
}

export const PricingGrid: React.FC<PricingGridProps> = ({features, classNames, ...props}) => {
  const router = useRouter();

  const slots = styles();

  const handleClick = (feat: PricingType) => {
    trackEvent("PricingGrid - Click", {
      name: feat.title,
      action: "click",
      category: "docs",
      data: feat.href ?? "",
  ...
  return (
    <div className={slots.base({class: classNames?.base})} {...props}>
      {features.map((feat: PricingType, index: number) => (
        <Card
